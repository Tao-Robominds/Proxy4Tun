...
config, param_file, family_mode = family_io.load_stage_params("sam")
print(f"[unified] stage=sam family_mode={family_mode}")
expected_keys = [
...
crop_taper_mm = float(config.get("crop_taper_mm", 1000 if geometry_profile == "t45" else 700))
print(
    f"unified SAM: geometry_profile={geometry_profile}, mirror_k={mirror_k_geometry}, "
    f"loop_extra={segment_loop_extra}, crop_taper_mm={crop_taper_mm}, y_bounds={y_bounds}"
)

# Complex-family geometric fallback (7-segment circular tiling). Gated on the
# family mode + segment count rather than the tunnel-id prefix.
_use_geometric_fallback = (
    (family_mode == "complex")
    and segment_per_ring == 7
    and bool(config.get("use_geometric_fallback", True))
)
_geometric_ky_offset = float(config.get("geometric_ky_offset", 0.0))

_denoise_params, _ = family_io.load_raw_params("denoising")
...
def fill_polygon(mask, vertices):
    ...
    mask[mask_inside] = 1
_mirror_k_geometry = mirror_k_geometry
...
def generate_template_mask(height, width, prompt_centre, block, resolution=0.005):
    ...
    if geometry_profile == "t45":
        vertices_real = t45_geometry.template_vertices_mm(x, y, block)
    elif geometry_profile == "t3":
        vertices_real = t3_geometry.template_vertices_mm(x, y, block)
    else:  # t12 (staggered)
        vertices_real = t12_geometry.template_vertices_mm(x, y, block)
    if block == 'K' and _mirror_k_geometry:
...
def generate_prompt_points(prompt_centre, initial_x, map_y, block, resolution,
    ...
    if geometry_profile == "t45":
        # Notebook T4/T5: geometry coords + x-bounds only (no T3 y_bounds label flip).
        points_real, labels = t45_geometry.prompt_points_mm(x, y, block)
        if block == 'K' and _mirror_k_geometry:
            points_real = _mirror_real_x_about_center(points_real, x)
        points = points_real / (resolution * 1000)
        within_bounds = (points[:, 0] >= 0) & (
            (points[:, 0] + initial_x - (segment_width * 0.5 + padding) / (resolution * 1000)) <= image.shape[1]
        )
        points = points[within_bounds]
        labels = labels[within_bounds]
        return points, labels

    if geometry_profile == "t3":
        # Notebook T3: geometry coords + y_bounds label flip with exceptions.
        points_real, labels = t3_geometry.prompt_points_mm(x, y, block)
        if block == 'K' and _mirror_k_geometry:
            points_real = _mirror_real_x_about_center(points_real, x)
        for i in range(len(labels)):
            if labels[i] == 0:
                ...
                y_exceptions = [y + float(off) for off in y_exceptions_offsets]
                y_exception_cond = points_real[i, 1] not in y_exceptions
                if y_cond and x_cond and y_cond2 and y_exception_cond:
                    labels[i] = 1
        points = points_real / (resolution * 1000)
        within_bounds = (points[:, 0] >= 0) & (
            (points[:, 0] + initial_x - (segment_width * 0.5 + padding) / (resolution * 1000)) <= image.shape[1]
    ...
    # t12 (staggered): inline t1&2 geometry (identical to t12_geometry tables)
    # followed by keep_mask negative-point removal.
    map_y = map_y_mm
...
def geometric_segment(detected_df, image_shape, ring_count, K_height, AB_height,
                      ...
                      reverse_circumferential=False):
    """Complex-tunnel (7-seg) fallback: label from detected K Y and circular tiling.

    ``reverse_circumferential`` flips the block walk around the ring — use with
    ``theta_sign=-1`` so the geometric tiler matches a mirrored unrolling.
    """
    H, W = image_shape[:2]
    ...
    downward_blocks = list(segment_order[1:]) if reverse_circumferential else list(reversed(segment_order[1:]))
    ...
    # Optional per-run Y nudge (pixels) to align the circular tiler with seams.
    ky_offset = float(globals().get("_geometric_ky_offset", 0.0))
...
def process_row(df_row, image, resolution, segment_per_ring, segment_width,
    ...
    results = []
    for i in range(segment_per_ring + segment_loop_extra):
        if reverse == False:
            block = block_labels[block_label_index]
            if block_label_index == 0:
                delta_y = convert_to_pixel_coords(0.5*K_height + math.tan(math.radians(angle))*crop_taper_mm+100 + crop_margin, resolution) # K-block
                map_y = initial_y
            else:
                delta_y = convert_to_pixel_coords(0.5*AB_height + math.tan(math.radians(angle))*crop_taper_mm+100 + crop_margin, resolution) # other block
...
if _use_geometric_fallback:
    ...
    reverse_circ = bool(config.get("reverse_circumferential", False))
    # Inherit theta_sign=-1 from unfolding params when SAM JSON omits the flag.
    if not reverse_circ:
        try:
            _uf, _ = family_io.load_raw_params("unfolding")
            reverse_circ = int(_uf.get("theta_sign", 1)) < 0
        except SystemExit:
            reverse_circ = False
    print(f"Using geometric fallback for complex tunnel {tunnel_id}")
    print(f"  segment_order={segment_order_geom}, reverse_circumferential={reverse_circ}, "
          f"K_px≈{int(round(K_height / (resolution * 1000)))}, "
    ...
    result_image, ring_image = geometric_segment(
        ...
        reverse_circumferential=reverse_circ,
    )
    fix_ring = np.where(
