...
from bo.paths import FULL_STAGE_PKG, NOTEBOOK_PKG, PROXY_SCALE_PKG

LOG_DIR = NOTEBOOK_PKG / "logs"
OUT_CSV = NOTEBOOK_PKG / "scores.csv"
OUT_MD = NOTEBOOK_PKG / "report.md"
...
def load_unified() -> dict[str, float]:
    path = FULL_STAGE_PKG / "holdout_scores.csv"
    out: dict[str, float] = {}
    with path.open() as f:
...
def load_refined() -> dict[str, float]:
    path = PROXY_SCALE_PKG / "refined_scores.csv"
    out: dict[str, float] = {}
    with path.open() as f:
        for r in csv.DictReader(f):
