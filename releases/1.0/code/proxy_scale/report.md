...
- Scoring: `score_scaled.py` = frozen pooled lean model (`bo/full_stage/models.json`)
...
- Stage-1 unlock (per `bo/full_stage/stage1_gate.md`): anchors with residual
...
`selections/<subset>_<arm>.json`; per-run artifacts `data/bo/reflect/<subset>/<arm>/`.
...
```bash
./venv/bin/python bo/proxy_scale/band_check.py                      # bands.json
./venv/bin/python bo/proxy_scale/run_subset.py --subset <s>         # cursor3 trios
./venv/bin/python bo/proxy_scale/select_round.py --subset <s> --arm <arm>
./venv/bin/python bo/proxy_scale/make_report.py                     # refined_scores.csv + summary
```
