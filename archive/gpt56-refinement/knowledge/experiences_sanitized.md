# Domain experience snapshot (sanitized for GPT-5.6 isolation)

Generic engineering lessons only. Target-specific reflection advice (§8),
campaign-specific proxy lessons (§11–12), Fable/Cursor campaign outputs,
and recipe-like overlays are excluded.
...
- Use the provided frozen scaled proxy as a **directional** improvement signal between rounds.
- Corroborate against artifacts themselves (line count, NaN fraction, orientation/permutation signatures, residual).
- A proxy increase driven only by evidence features while coherence features degrade is suspect.
