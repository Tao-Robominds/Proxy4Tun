...
class RANSAC:
    ...
    def execute_ransac(self):
        ...
        # Cap iterations: the legacy loop never decremented `items`, so a plane that
        # never reaches max_inliers could spin forever (seen on T4 remove_top_tube).
        ...
        while remaining > 0:
            ...
            if inliers_count > self.count:
                ...
                # Recompute remaining iterations from current inlier ratio
        ...
        if self.count <= self.max_inliers:
            print(
                f"Inlier ratio (best, below target): {self.count / max(len(self.point_data), 1):.4f}"
            )
...
@njit
def calculate_angle_with_direction(A, B, C, T, deterministic_orientation):
    ''' 
...
print(
    ...
    f"median centreline Tz={np.median(T_vectors[:, 2]):.5f}"
)
...
def process_batch(points_batch):
    ...
    for i in range(points_batch.shape[0]):
        ...
        angle_ABC, distance_AB = calculate_angle_with_direction(
            A, B, C, T, deterministic_theta_orientation
        )
