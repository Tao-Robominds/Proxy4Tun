...
cd "$(dirname "$0")/../.."
PY=./venv/bin/python
LOG=data/bo/full_stage/campaign.log
mkdir -p data/bo/full_stage
...
p=Path("data/bo/full_stage/${case}-trials/manifest.json")
if not p.exists():
    print(0)
else:
...

{
  ...
  if [ "$need21" -gt 0 ]; then
    $PY bo/full_stage/run_trials_full.py --case 2-1 --n "$need21" --start-index 0 --seed 42
  fi

  need31=$((10 - $(count_ok 3-1)))
  echo "=== $(date -Is) start 3-1 need=$need31 ==="
  if [ "$need31" -gt 0 ]; then
    $PY bo/full_stage/run_trials_full.py --case 3-1 --n "$need31" --start-index 1 --seed 43
  fi

  need51=$((19 - $(count_ok 5-1)))
  echo "=== $(date -Is) start 5-1 need=$need51 ==="
  if [ "$need51" -gt 0 ]; then
    $PY bo/full_stage/run_trials_full.py --case 5-1 --n "$need51" --start-index 0 --seed 44
  fi
