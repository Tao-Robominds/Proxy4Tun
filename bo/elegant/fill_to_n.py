...
from bo.paths import REPO_ROOT, ELEGANT_DATA, ELEGANT_PKG
from bo.elegant.features import TRAIN_ANCHORS
from bo.elegant.run_trials import _sobol_overlays, run_trial
from bo.unified.spaces import CASE_CONFIG
from bo.unified.intrinsics import extract_intrinsics, TIER1_KEYS

TRAIN_CSV = ELEGANT_PKG / "family" / "training_table.csv"
V1_FEATS = [
    "depth_nan_ratio",
...
def _max_trial_index(case: str) -> int:
    man_path = ELEGANT_DATA / f"{case}-trials" / "manifest.json"
...
def _row_from_trial(case: str, rec: dict[str, Any]) -> dict[str, Any] | None:
    ...
    params_dir = ELEGANT_DATA / f"{case}-trials" / "params" / rec["trial_id"]
