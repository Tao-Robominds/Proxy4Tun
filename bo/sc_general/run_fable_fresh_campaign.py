#!/usr/bin/env python3
"""Resumable Fable 5.1 Stage-3 campaign orchestrator (arm=fable_fresh).
...
claude-fable-5-1-thinking-high subagent per case (resumed for rounds 2–3).

Usage:
  ./venv/bin/python bo/sc_general/run_fable_fresh_campaign.py --derive-panel
  ./venv/bin/python bo/sc_general/run_fable_fresh_campaign.py --emit 3-4
  ./venv/bin/python bo/sc_general/run_fable_fresh_campaign.py --apply 3-4 --round 1 \
      --proposal-json data/sc-general/stage3/packets-fable-fresh/3-4/round1_proposal.json
...
CAMPAIGN = STAGE3 / "campaign_fable_fresh.md"
...
AGENT_STATE = STAGE3 / "fable_fresh_agent_state.json"
FREE_PAUSE_GB = 50.0
KEEP_ROUND_NAMES = {
...
def emit_packet(subset: str, *, write_overlay: bool = True) -> dict[str, Any]:
    ...
    prompt = f"""# Fable 5.1 reflection — subset {subset} (propose round {next_round})
...
def _gpu_lock() -> Any:
    """Serialize pipeline applies across concurrent case runners."""
    import fcntl

    STAGE3.mkdir(parents=True, exist_ok=True)
    lock_path = STAGE3 / "fable_fresh_gpu.lock"
...
def apply_proposal(
    ...
    *,
    ...
    lock_fh = _gpu_lock()
    try:
        return _apply_proposal_locked(subset, round_id, proposal, provenance=provenance)
    finally:
        try:
            import fcntl
...
def main() -> None:
    ...
    if args.emit:
        ...
        return
    if args.apply:
        # Hard gate: rogue Task subagents were stampeding --apply. Only the
        # serial apply queue (or an explicit operator) may set this env var.
        if os.environ.get("FABLE_FRESH_ALLOW_APPLY") != "1":
            raise SystemExit(
                "--apply blocked: set FABLE_FRESH_ALLOW_APPLY=1 "
                "(serial apply queue only; propose-only agents must not apply)"
            )
