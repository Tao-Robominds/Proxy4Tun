...
(`recentre_residual_max_cm` 20.8 > 10, per `bo/full_stage/stage1_gate.md`).

## Scaled-proxy verification (pre-gate)

- `./venv/bin/python bo/proxy_scale/band_check.py` → `bo/proxy_scale/bands.json`
...
- Live scorer cross-check: `score_scaled.py --subset 3-4 --anchor` → 0.5737,
  matches `bo/full_stage/holdout_scores.csv` (0.574).
...
Full overlays: `data/bo/reflect/3-4/cursor3/round*/reflection_record.json`.
Selection record: `bo/proxy_scale/selections/3-4.json`.
